...
public class MemPoolTest extends TestCase
{
    ...
    {
        mp.insert("Nonstop".getBytes(),
                ((short) "Nonstop".getBytes().length));
        assertEquals(mp.get(new MemHandle(0)), "Nonstop");

        mp.insert("No Role Modelz".getBytes(),
                ((short) "No Role Modelz".getBytes().length));
        assertEquals(
                mp.get(new MemHandle("Nonstop".getBytes().length + 2)),
                "No Role Modelz");

        mp.insert("Love yourz".getBytes(),
                ((short) "Love yourz".getBytes().length));
        assertEquals(
                mp.get(new MemHandle("Nonstop".getBytes().length
                        + "No Role Modelz".getBytes().length + 4)),
                "Love yourz");
    ...
    public void testRightFB()
    {
        ...
        mp.insert("Modelz".getBytes(),
                ((short) "Modelz".getBytes().length));
        assertEquals(mp.get(new MemHandle("Non".getBytes().length + 2)),
                "Modelz");

        // will have to expand to fit Love yourz
        mp.insert("Love".getBytes(), ((short) "Love".getBytes().length));
        assertEquals(mp.get(new MemHandle(
                "Non".getBytes().length + "Modelz".getBytes().length + 4)),
                "Love");
    ...
    /**
     * tests removal when only one thing is present, and when it isn't
     */
